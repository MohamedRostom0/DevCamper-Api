...
exports.login = asyncHandler(async (req, res, next) => {
  ...
  if (!email || !password) {
    return next(
      new ErrorResponce("Please provide an email and a password", 400)
    );
  }
  ...
  if (!isMatch) return next(new ErrorResponce("Invalid credentials", 401));
...
exports.forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ErrorResponce(`No user with email: ${req.body.email}`, 404)
    );
  }
