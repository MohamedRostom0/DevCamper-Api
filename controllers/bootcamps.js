...
exports.createBootcamp = asyncHandler(async (req, res, next) => {
    ...
    // If the user is not an admin; he can only add 1 bootcamp
    if(publishedBootcamp && req.user.role !== 'admin'){
        return next(new ErrorResponce(`The user with id ${req.user.id} has already published a bootcamp`, 400))
    }
...
exports.updateBootcamp = asyncHandler(async (req, res, next) => {
    ...
    // Make Sure logged in user owns the bootcamp
    if(bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin'){
        return next(new ErrorResponce(`User with id: ${req.params.id} is not authorized to update this bootcamp`, 401))
    }
...
exports.deleteBootcamp = asyncHandler (async (req, res, next) => {
    ...
    if(bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin'){
        return next(new ErrorResponce(`User with id: ${req.params.id} is not authorized to delete this bootcamp`, 401))
    }
